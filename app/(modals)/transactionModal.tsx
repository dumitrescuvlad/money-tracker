  ...
  const onDateChange = (event: any, selected?: Date) => {
    if (Platform.OS === "android") {
      if (event.type === "set" && selected) {
        // user tapped OK
        setTransaction((prev) => ({ ...prev, date: selected }));
      }
      // close the Android dialog both on OK and on Cancel
      setShowDatePicker(false);
    } else {
      // iOS (inline/spinner) updates live; keep it open until "Done"
      if (selected) setTransaction((prev) => ({ ...prev, date: selected }));
    }
  };
  /*
  useEffect(() => {
      ...
      <View style={styles.footer}>
        ...
        <Button onPress={onSubmit} loading={loading} style={{ flex: 1 }}>
          <Typo color={colors.black} fontWeight={"700"}>
            {oldTransaction?.id ? "Update Wallet" : "Create Wallet"}
          </Typo>
        </Button>
      </View>
    </ModalWrapper>
