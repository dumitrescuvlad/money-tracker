...
const revertAndUpdateWallets = async (
  ...
  try {
    ...
    if (newTransactionType === "expense") {
      // if user tries to convert an income to an expense on the same wallet
      ...
      if (
        ...
        return {
          success: false,
          msg: "The selected wallet doesn't have enough balance for this transaction",
        };
      }
      // if user tries to add expense froma new wallet but the wallet doesn't have enough balance
    ...
    // refetch the new wallet after updating it
...
export const deleteTransaction = async (
  ...
  try {
    ...
    //check fiels to be updated on transaction type
    const updateType =
...
export const fetchWeeklyStats = async (uid: string): Promise<ResponseType> => {
  try {
    ...
    // takes each day and create 2 entries in an array one for income and one for expense
  ...
  try {
    ...
    querySnapshot.forEach((doc) => {
      ...
      const yearData = yearlyData.find(
        (item: any) => item.year === transactionDate.toString()
      );
